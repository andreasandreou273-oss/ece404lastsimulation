...
WAYPOINT_REACH_THRESHOLD_M = 6.0
    ...
    def wait_until_close(self, target: Position, threshold_m: float = WAYPOINT_REACH_THRESHOLD_M) -> Position:
        ...
        while True:
            if time.time() - last_command_time >= 2.0:
                # Refresh the target command periodically so the vehicle keeps tracking the waypoint.
                self.goto(target)
...
def load_buildings(buildings_file: Path) -> List[BuildingPlan]:
    ...
    for line_number, raw_line in enumerate(buildings_file.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        buildings.append(
            BuildingPlan(
                name=parts[0],
                center=Position(lat=float(parts[1]), lon=float(parts[2]), alt=float(parts[3])),
    ...
    if len(buildings) < 3:
        raise ValueError("This scenario needs at least 3 buildings in the buildings file")
...
def load_exact_corners(corners_file: Path, buildings: Sequence[BuildingPlan]) -> dict[str, List[CornerPlan]]:
    ...
    for line_number, raw_line in enumerate(corners_file.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        for index, corner_name in enumerate(["P1", "P2", "P3", "P4"]):
            corners.append(
                CornerPlan(
                    building_name=name,
...
def scan_building(
    ...
    try:
        prepare_for_mission(drone, building.center.alt, event_logger)
        event_logger.log(
            "building_start",
            f"{drone.name} starting rectangle scan for {building.name}",
            drone=drone.name,
            building=building.name,
        )

        for corner in corners:
            print(f"\n{drone.name}: flying to {building.name} {corner.corner_name}")
            event_logger.log(
                "scan_move",
                f"{drone.name} flying to {building.name} {corner.corner_name}",
                drone=drone.name,
                building=building.name,
            ...
            drone.goto(corner.position)
            drone.wait_until_close(corner.position)

            print(f"{drone.name}: scanning at {building.name} {corner.corner_name} for {scan_delay_s:.1f} s")
            event_logger.log(
                "scan_start",
                f"{drone.name} scanning {building.name} {corner.corner_name}",
                drone=drone.name,
                building=building.name,
                corner=corner.corner_name,
            ...
            print(
                f"{drone.name}: {building.name} {corner.corner_name} | "
            ...
            event_logger.log(
                "scan_result",
                (
                    f"{drone.name} scan result at {building.name} {corner.corner_name}: "
                    f"probability={corner.probability:.0%}, random={random_value:.3f}, "
                    f"detected={'YES' if detected else 'NO'}"
                ),
                drone=drone.name,
            ...
            if detected:
                print(
                    f"{drone.name}: person detected at lat={corner.position.lat:.6f}, "
                    f"lon={corner.position.lon:.6f}"
                )
                event_logger.log(
                    "person_detected",
                    f"{drone.name} detected a person at {building.name} {corner.corner_name}",
                    drone=drone.name,
                    building=building.name,
                    corner=corner.corner_name,
                    lat=corner.position.lat,
                    lon=corner.position.lon,
                    alt=corner.position.alt,
                )

        event_logger.log(
            "building_complete",
            f"{drone.name} completed rectangle scan for {building.name}",
            drone=drone.name,
            building=building.name,
        )
    except Exception as exc:  # pragma: no cover - live mission defensive path
        message = f"{drone.name}: {exc}"
        errors.append(message)
        print(message)
        event_logger.log("mission_error", message, drone=drone.name, building=building.name)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Three drones each scan one building and log person detection coordinates."
    )
    ...
    print("Three-drone building assignments:")
    assignments = list(zip(drone_names, buildings))
    for drone_name, building in assignments:
        corner_mode = "exact corners from file" if building.name in exact_corners_by_building else "generated corners"
        print(
            f"- {drone_name} -> {building.name}: center=({building.center.lat:.6f}, {building.center.lon:.6f}), "
            f"alt={args.altitude:.1f} m, probability={building.probability:.0%}, {corner_mode}"
        )

    event_logger.log(
        "mission_start",
        "Three-drone building scan mission started",
        assignments=[
            {
                "drone": drone_name,
                "building": building.name,
    ...
    for index, (drone_name, building) in enumerate(assignments):
        building_with_altitude = BuildingPlan(
            name=building.name,
            center=Position(lat=building.center.lat, lon=building.center.lon, alt=args.altitude),
            probability=building.probability,
            half_north_m=building.half_north_m,
            half_east_m=building.half_east_m,
        )
        corners = exact_corners_by_building.get(building.name, build_generated_corners(building_with_altitude))
        if building.name in exact_corners_by_building:
            corners = [
                CornerPlan(
                    building_name=corner.building_name,
                    corner_name=corner.corner_name,
                    position=Position(lat=corner.position.lat, lon=corner.position.lon, alt=args.altitude),
                    probability=corner.probability,
                )
                for corner in corners
            ]

        drone = DroneConnection(drone_name, DRONE_PORTS[drone_name])
        thread = threading.Thread(
            target=scan_building,
            args=(
                drone,
                building_with_altitude,
                corners,
                args.scan_delay,
                event_logger,
    ...
    print("\nMission summary:")
    for attempt in sorted(attempts, key=lambda item: (item.drone_name, item.building_name, item.corner_name)):
        print(
            f"- {attempt.drone_name}: {attempt.building_name} {attempt.corner_name} | "
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        event_logger.log("mission_complete", "Three-drone mission completed with errors", errors=errors)
    else:
        event_logger.log("mission_complete", "Three-drone mission completed successfully")
